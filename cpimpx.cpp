...
void CPI::insertBndcu(StoreInst *LI, CallInst* CI, int priority) {
  ...
  auto ptrOpAsVoidPtr =
      IRB.CreateBitCast(realFunction, Type::getInt64PtrTy(CI->getContext()), "");
  auto checkTy =
...
void CPI::insertBndldx(StoreInst *SI, int priority) {
  ...
  auto ckBound = InlineAsm::get(checkTy, "bndldx ($0, $1, 1), " + bndReg,
                                "r,~{dirflag}, ~{fpsr}, ~{flags}", true);
  IRB.SetInsertPoint(SI->getNextNode());
  IRB.CreateCall(ckBound, {ptrOpAsVoidPtr, ptrOpAsVoidPtr});
}
*/
void CPI::insertBndStx(StoreInst *, int priority) {
  _DI.numBndStx++;
  std::string bndReg;
  switch (priority) {
    assert(priority > -1 && priority < 4);
  ...
  auto ckBound = InlineAsm::get(checkTy, "bndldx " + bndReg + ", ($0, $1, 1)",
                                "r,~{dirflag}, ~{fpsr}, ~{flags}", true);
  IRB.SetInsertPoint(SI->getNextNode());
  IRB.CreateCall(ckBound, {ptrOpAsVoidPtr, ptrOpAsVoidPtr});
}
